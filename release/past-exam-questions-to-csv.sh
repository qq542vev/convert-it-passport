#! /bin/sh
# Wrapper script to run past-exam-questions-to-csv.sh with bundled modernish
...
case $MSH_SHELL in
( */sh )	;;
( * )	echo past-exam-questions-to-csv.sh: "warning: sh shell not usable; running on $MSH_SHELL" >&2 ;;
esac

# Prefix launch arguments.
set -- "$MSH_PREFIX"/bin/past-exam-questions-to-csv.sh "$@"
