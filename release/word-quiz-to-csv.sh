#! /bin/sh
# Wrapper script to run word-quiz-to-csv.sh with bundled modernish
...
case $MSH_SHELL in
( */sh )	;;
( * )	echo word-quiz-to-csv.sh: "warning: sh shell not usable; running on $MSH_SHELL" >&2 ;;
esac

# Prefix launch arguments.
set -- "$MSH_PREFIX"/bin/word-quiz-to-csv.sh "$@"
